...
d$dist.e.scaled <- (d$dist.e - mean.dists)/sd.dists
d$dist.a.scaled <- (d$dist.a - mean.dists)/sd.dists
d$dist.o.scaled <- (d$dist.o - mean.dists)/sd.dists
# Record stratigraphic relations 
constraint.text  <- paste0('chronological.constraint ~ dconstraint(',theta.constraints,')')

save(d,constraint.text,file=here('data','input_binom.RData'))
